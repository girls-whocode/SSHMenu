...
#--------------------------{ Default values }---------------------------------------------------------------------------
     OPT=$1     # Options.
     KEY=~/.ssh/nasvc_orion.pub # SSH key to use in command Sshkey(Add my ssh key to host).
CONFILES=$(shopt -s nullglob; echo ~/.ssh/{config,config*[!~],config*[!~]/*})   # SSH confiles list.
  REMOTE=8080   # Remote port for tunneling.
   LOCAL=18080  # Local  port for tunneling.
   GUEST=nasvc_orion  # Alternative username to login with.
...
group_id=group  # Group separator identification template, 'dummy' by default, don't ask why, historically)
...
sshto_script[0]=~/.local/bin/sshmenu # Path to sshto script
sshto_script[1]=ssh_script.sh  # Sshto script filename
...
cmdlist_renew(){
    ...
    cmdlist_group=(
        ...
        "Sshkey"     "Add my ssh key to \Z4$group\Z0"
        ''           ''
        "ls  -la"    "List Files"
        "free -h"    "Show free memory"
        "df  -ih"    "Show free inodes"
